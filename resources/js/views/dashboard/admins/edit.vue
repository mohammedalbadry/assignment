  ...
  <div class="card card-primary card-outline">
    <div class="card-body box-profile">
        ...
        <div class="mb-3">
              ...
              <select 
                  v-model="form.role"
                 :class="(v$.form.role.$error || serverside_error.role) ? 'is-invalid' : '' "
              ...
              <div class="invalid-feedback" v-if="v$.form.role.$error">{{ v$.form.role.$errors[0].$message }}</div>
              <div class="invalid-feedback" v-if="serverside_error.role">{{ serverside_error.role[0] }}</div>
        </div>
...
export default {
  name: 'editadmin',
    ...
    methods:{
        ...
        read:function(){
            axios({ url: '/admins/'+ this.$route.params.id, method: 'get' })
            .then(response=> {
                this.form = response.data.data
                this.form.role = response.data.data.roles[0].id
            })
            .catch( error => {/*console.log(error)*/});
        },
        async update(){
            ...
            if(this.form.role) { fd.append('role',this.form.role) }
            if(typeof(this.form.photo) == "object"){
              fd.append('photo', this.form.photo)
            }

            axios({ url: '/admins/'+ this.$route.params.id, method: 'post', data: fd})
            .then(response=> {
