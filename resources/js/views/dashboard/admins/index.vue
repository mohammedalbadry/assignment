                ...
                <tbody>
                     <tr v-for="(item, index) in allItem.data" :key="index">
                        ...
                        <td>
                            ...
                            <router-link v-if="$store.state.AdminPermission.includes('admin-update')" :to="'admins/edit/' + item.id" class="btn btn-primary btn-sm">
                                edit
                            </router-link>
                            <router-link v-if="$store.state.AdminPermission.includes('admin-read')" :to="'admins/show/' + item.id" class="btn btn-success btn-sm">
                                show
                            </router-link>
                            <button v-if="$store.state.AdminPermission.includes('admin-delete')" @click="passActionItem(item)" type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#delete_modal">
...
export default {
    ...
    methods:{
        ...
        destroy:function(){
            ...
            axios({ url: '/admins/' + this.actionItem.id, method: 'post', data: fd})
            .then(response=> {
