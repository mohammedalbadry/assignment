            ...
            <table class="table table-bordered">
                ...
                <tbody>
                     <tr v-for="(item, index) in allItem.data" :key="index">
                        <td>{{item.name}}</td>
                        <td>
                            <router-link v-if="$store.state.AdminPermission.includes('users-update')" :to="'users/edit/' + item.id" class="btn btn-primary btn-sm">
                                edit
                            </router-link>
                            <router-link v-if="$store.state.AdminPermission.includes('users-read')" :to="'users/show/' + item.id" class="btn btn-success btn-sm">
                                show
                            </router-link>
                             <router-link v-if="$store.state.AdminPermission.includes('timetable-create')" :to="'timetables/create/user/' + item.id" class="btn btn-info btn-sm">
                                new time
                            </router-link>
                            <button v-if="$store.state.AdminPermission.includes('users-delete')" @click="passActionItem(item)" type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#delete_modal">
...
export default {
    ...
    methods:{
        ...
        destroy:function(){
            ...
            axios({ url: '/users/' + this.actionItem.id, method: 'post', data: fd})
            .then(response=> {
