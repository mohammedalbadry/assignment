    ...
    <div class="card card-outline card-primary">
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-md-7">
                    <router-link to="timetables/create" class="btn btn-primary">
                        add
                    </router-link>
                </div>
            </div>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                     <tr v-for="(item, index) in allItem.data" :key="index">
                        ...
                        <td>{{item.title}}</td>
                        <td>
                            <router-link v-if="$store.state.AdminPermission.includes('timetable-update')" :to="'timetables/edit/' + item.id" class="btn btn-primary btn-sm">
                                edit
                            </router-link>
                            <router-link v-if="$store.state.AdminPermission.includes('timetable-read')" :to="'timetables/show/' + item.id" class="btn btn-success btn-sm">
                                show
                            </router-link>
                            <button v-if="$store.state.AdminPermission.includes('timetable-delete')" @click="passActionItem(item)" type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#delete_modal">
            ...
            <div class="modal fade" id="delete_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    ...
                    <div class="modal-body">
                        <p>Confirm deletion of <strong>{{actionItem.title}}</strong></p>
                        <div class="text-danger" v-if="serverside_error.prevent">{{ serverside_error.prevent }}</div>
                    </div>
...
export default {
    ...
    methods:{
        ...
        destroy:function(){
            ...
            axios({ url: '/timetables/' + this.actionItem.id, method: 'post', data: fd})
            .then(response=> {
